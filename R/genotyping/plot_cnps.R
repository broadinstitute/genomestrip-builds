...
current.sample.group.data <- NULL
...
plotSites <- function(siteList, threshold=1.3, ymax=NULL, impMethod=NULL) {
    ...
    current.sample.group.data <<- loadSampleGroups()
...
plotCnpInternal <- function(cnp, coverage.data, count.data, expected.data, genotype.data, confidence.data, pair.data, model, norm.data, truth.data, sample.group.data=NULL, sample.data=NULL, excluded.site.samples=NULL, gender.data=NULL, threshold=NULL, ymax=NULL) {
    ...
    if (substr(genotype.data$CHR,1,3) == "chr") {
        siteInterval <- sprintf("%s %d-%d", genotype.data$CHR, genotype.data$LEFTEND, genotype.data$RIGHTSTART - 1)
    } else {
        siteInterval <- sprintf("chr%s %d-%d", genotype.data$CHR, genotype.data$LEFTEND, genotype.data$RIGHTSTART - 1)
    }
    title2 <- siteInterval
    if (cnp.length >= 1000) {
    ...
    if (is.null(partition)) {
        ...
        if (!is.null(partitionMap)) {
            ...
            if (!is.null(partitionId) && !is.na(partitionId) && !is.null(cmdArguments$runDirectory)) {
                if (substr(partitionId,1,1) == "/") {
                    # listed partition is full absolute path
