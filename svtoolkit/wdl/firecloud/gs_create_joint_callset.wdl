...
task reclassifyDeletions {
    ...
    Int adjustedDiskSize = diskSize + 2 * (round(size(delCallset, "G") + round(size(cnvCallset, "G"))) + 10)

    command {
        $SV_DIR/scripts/firecloud/joint/reclassify_del_sites.sh ${delCallset} ${cnvCallset} ${referenceBundle}
    }

    output {
        File reclassifiedSitesVcf = "reclassified.sites.vcf.gz"
    }
...
task runParallelGenotyper {
    ...
    Int adjustedDiskSize = diskSize + 2 * (round(size(delOutputPath, "G")) + 2)
    ...
    command {
        ...
        $SV_DIR/scripts/firecloud/genotyping/run_parallel_genotyper.sh ${vcfFile} ${partitionName} "$(cat partition.arg)" ${mdPath} ${bamFileList} ${referenceBundle} true ${credentialsKeyFile}
    }
...
task mergeBatchPartitions {
    ...
    command {
        ...
        java -cp $SV_CLASSPATH -Xmx4g \
            ...
            -O batch.genotypes.vcf.gz \
            || exit 1
    }

    output {
        File batchVcf = "batch.genotypes.vcf.gz"
    }
...
task mergeReclassifiedBatches {
    ...
    command {
        ...
        $SV_DIR/scripts/firecloud/joint/merge_reclassified_batches.sh vcf_files.list ${referenceBundle}
    }

    output {
        File reclassifiedGenotypesVcf = "reclassified.genotypes.vcf.gz"
    }
...
task mergeJointCalls {
    File delCallset
    File cnvCallset
    File reclassifiedGenotypesVcf
    File referenceBundle
    ...
    Int adjustedDiskSize = diskSize + round(2.2 * (size(delCallset, "G") + size(cnvCallset, "G")) + size(reclassifiedGenotypesVcf, "G"))
    
    command {
        $SV_DIR/scripts/firecloud/joint/merge_joint_calls.sh ${delCallset} ${cnvCallset} ${reclassifiedGenotypesVcf} ${referenceBundle}
    }

    output {
        File chromsListFile = "chroms.list"
        File jointCallsVcf = "joint_callset.tar.gz"
        File jointCallsVcfIndex = "joint_callset.tar.gz.tbi"
    }
...
task setupRedundancyFiltering {
    ...
    command {
        $SV_DIR/scripts/firecloud/common/create_genome_partitions.sh ${chromsListFile} ${windowSize} intervals.list ${referenceBundle}
    }
...
task filterRedundantSites {
    ...
    command {
        ...
        java -cp $SV_CLASSPATH -Xmx4g \
            ...
            -vcf ${vcfFile} \
            -L ${interval} \
            -O gs_joint.genotypes.vcf.gz \
            || exit 1
    }

    output {
        File dedupedVcf = "gs_joint.genotypes.vcf.gz"
    }
...
task createFinalCallset {
    ...
    command {
        ...
        $SV_DIR/scripts/firecloud/joint/create_joint_callset.sh vcf_files.list ${referenceBundle}
    }

    output {
        File jointCallset = "joint_callset.tar.gz"
    }
...
workflow gs_create_joint_callset_wf {
    File batchInfo
    File delCallset
    File cnvCallset
    ...
    call reclassifyDeletions {
        input:
            delCallset = delCallset,
            cnvCallset = cnvCallset,
    ...
    call setupGenotyping {
        input:
            vcfFile = reclassifyDeletions.reclassifiedSitesVcf,
    ...
    call mergeJointCalls {
        input:
            delCallset = delCallset,
            cnvCallset = cnvCallset,
            reclassifiedGenotypesVcf = mergeReclassifiedBatches.reclassifiedGenotypesVcf,
            referenceBundle = referenceBundle,
            diskSize = diskSize
    }
    ...
    call setupRedundancyFiltering {
        input:
            chromsListFile = mergeJointCalls.chromsListFile,
            referenceBundle = referenceBundle,
            diskSize = diskSize,
            numPreempt = numPreempt
    }
    ...
    scatter(interval in setupRedundancyFiltering.intervalsList) {
        call filterRedundantSites {
            input:
                vcfFile = mergeJointCalls.jointCallsVcf,
                vcfFileIndex = mergeJointCalls.jointCallsVcfIndex,
                interval = interval,
                referenceBundle = referenceBundle,
                diskSize = diskSize,
                numPreempt = numPreempt
    ...
    output {
        File jointCallset = createJointCallset.jointCallset
    }
}
